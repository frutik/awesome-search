...
- Build the URL from the note's path relative to `obsidian/vault/Awesome Search/`, **without the `.md` extension**, URL-encoded (spaces → `%20`, `&` → `%26`), prefixed with the site base. E.g. `[[Bayesian BM25]]` → `[Bayesian BM25](https://frutik.github.io/awesome-search/Concepts/Bayesian%20BM25)`; `[[Note|alias]]` uses `alias` as display text.
