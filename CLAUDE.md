...
- `mails/` — generated per-week HTML email digests of the History log, one
  ...
  never hand-edited.
- `drafts/` — long-form standalone essays written by hand as plain Markdown,
  outside the vault and outside the KG pipeline. `drafts/html/` holds their
  rendered HTML versions, generated by the `kg-draft-to-html` skill and never
  hand-edited.
...
- **`kg-draft-to-html`** — renders one hand-written essay from `drafts/*.md`
  into a self-contained page at `drafts/html/<stem>.html`, same house style
  as the mail digests, via
  `claude-skills/kg-draft-to-html/scripts/draft_to_html.py`. Strictly a
  format conversion: unlike `kg-mail-list` it derives nothing and edits
  nothing — no rewriting, no stat-stripping, and `[[wikilinks]]` are left
  verbatim rather than resolved to site links. Not a vault-writing skill.
...
- **`kg-audit`** — the same four checks plus duplicate-source articles,
  unresolved wikilinks, and a README-sync check (is `README.md` still
  derived-consistent with `global_toc.md` and the newest History weeks?), in
  one pass via `claude-skills/kg-audit/scripts/
